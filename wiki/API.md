...
```
 trusted server modules (Market, Trading, Rewards, ...)
            ...
            ▼
   ┌──────────────────────┐  delegates   ┌──────────────────────┐      writes balances        ┌─────────────────┐
   │   EconomyProvider     │ ───────────▶ │   CurrencyService     │ ──────────────────────────▶ │  in-memory      │
   │  (single entry point) │              │  (server core API)    │                              │  Wallet (Profile)│
   └──────────────────────┘              └──────────┬───────────┘                              └────────┬────────┘
...
1. **`EconomyProvider` (`src/server/EconomyProvider.luau`) is the only file
   trusted server modules require.** It is a thin facade over `CurrencyService`
   and `CurrencyConfig`; treat those (and `ProfileStore`, `Replication`,
   `Validation`) as internals you never require directly.
2. **All balance mutations happen on the server**, only through `EconomyProvider`.
3. **The client is read-only.** It observes state via `WalletController`; it
...
4. **Register custom currencies at boot**, before any player profile loads.
5. Every mutation is validated (NaN, infinity, negative, non-integer, overflow,
...
| `src/server` | `ServerScriptService.Server` | Bootstrap + `EconomyProvider` (the entry point) + its internals |
...
## 3. Server API — `EconomyProvider`

`EconomyProvider` (`src/server/EconomyProvider.luau`) is the **only** file
trusted server modules require. It is a thin facade over `CurrencyService`
and `CurrencyConfig` — those, plus `ProfileStore`, `Replication`, and
`Validation`, are internal implementation detail you never require directly.
...
Server-side convenience proxy for `CurrencyConfig.register` (see
[§4](#4-shared-api--currencyconfig)) — call this instead of reaching into
`src/shared` from a server script. The client has no `EconomyProvider`; it
still registers custom currencies through `CurrencyConfig` directly.

### `EconomyProvider.getCurrencyDefinition(id)` / `EconomyProvider.getAllCurrencies()`

Server-side convenience proxies for `CurrencyConfig.get` / `CurrencyConfig.all`
(see [§4](#4-shared-api--currencyconfig)).
...
not exposed through `EconomyProvider` since consumers never call them directly:
...
| :--- | :--- |
| `EconomyProvider` | `ServerScriptService.EconomyServer.EconomyProvider` |
...
local function buy(player: Player, itemPrice: number)
    local result = EconomyProvider.minusMoney(player.UserId, "gold", itemPrice)
    if result.ok then
        grantItem(player)
        return true
    end
...
| Add a new currency type (server) | `EconomyProvider.registerCurrency({ id = ... })` (at boot) | Server |
| Add a new currency type (client) | `CurrencyConfig.register({ id = ... })` (at boot) | Client |
| List all currencies | `CurrencyConfig.all()` (or `EconomyProvider.getAllCurrencies()` server-side) | Both |
| Look up a currency's definition | `CurrencyConfig.get(id)` (or `EconomyProvider.getCurrencyDefinition(id)` server-side) | Both |
